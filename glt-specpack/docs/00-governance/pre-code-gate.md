...
- [x] Правила границ выполняются автоматически, а не на ревью — S-1, S-2, S-5, S-6 в CI
- [x] Типы событий генерируются из реестра — кодогенерация, литерал ловится сканом (S-3)
- [ ] Каждая запись реестра имеет **работающую** проверку в CI — 7 из 10: S-4, S-7, S-9 ждут DEV-04
- [ ] Каждый инвариант имеет тест со своим ID — DEV-04

Отмеченные пункты подтверждены не наличием кода, а негативным прогоном: нарушение вносится намеренно, и проверка обязана упасть. Отмечать до этого запрещено — это ровно тот антипаттерн, из-за которого «DAG ацикличен» стоял пройденным при живом цикле.
