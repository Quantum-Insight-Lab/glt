...
- [x] Escape-сценарии E06 блокируются полностью, 100%
- [x] Сеть выключена по умолчанию, egress только через policy broker
- [x] Код проверяемой ветки считается недоверенным
- [x] Нет Docker socket, нет host credentials, образ закреплён подписанным digest
- [x] Seeded failures S4–S6 автоматизированы
...
**сделано** — ветка `wave4`. E06.1–6 отвергаются; сеть off без grant; ветка untrusted; нет socket и host credentials; образ — digest конверта; S4–S6 в CI. CI: [run](https://github.com/Quantum-Insight-Lab/glt/actions/runs/35062323777).
