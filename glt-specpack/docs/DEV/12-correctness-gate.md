...
- [x] E02a: recall обязательных проверок равен 1.0 на всём наборе
- [x] E02b: recall затронутых узлов равен 1.0 на всём наборе
- [x] E03: ноль false-green на holdout H01–H05
- [x] E05a: изменение вне boundary всегда даёт непустой `known_unknowns`
- [x] Precision посчитана и опубликована, но не гейтится

### Честность прогона

- [x] Прогон идёт на мета-графе, а не на четырёхузловом срезе
- [x] Holdout не использовался для отладки и настройки параметров
- [x] Отчёт называет размер набора и версии classifier и matrix
- [x] Отчёт нигде не утверждает, что GLT ускоряет работу
- [x] Usefulness gate остаётся в статусе «не проверено», а не «пройден»
...
**сделано** — ветка `wave1`. Recall 1.0 на `glt.seeded-slice@1` (13 случаев). Holdout H01–H05 без false-green. Precision публикуется. Usefulness остаётся «не проверено». CI: [run](https://github.com/Quantum-Insight-Lab/glt/actions/runs/34438085012).
