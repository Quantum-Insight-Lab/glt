---
id: glt.dev.01
owner: engineering
normativity: normative
status: accepted
wave: 1
depends_on: []
spec_refs:
  - ../SPEC/architecture.md
  - ../SPEC/structural-invariants.md
  - ../SPEC/cli.md
  - ../../../AGENTS.md
risk: low
gate: none
source_refs:
  - repository: glt-controlplane
    path: glt-specpack/docs/SPEC/architecture.md
    authority: engineering-contract
    role: derived-from
  - repository: glt-controlplane
    path: glt-specpack/docs/SPEC/structural-invariants.md
    authority: structural-invariants
    role: derived-from
  - repository: glt-controlplane
    path: glt-specpack/docs/SPEC/cli.md
    authority: engineering-contract
    role: derived-from
  - repository: glt-controlplane
    path: AGENTS.md
    authority: structural-invariants
    role: derived-from
---

# 01 — Каркас репозитория

**Волна:** 1 · **Риск:** низкий · **Gate:** нет

## Что делаем

- Скелет `control-plane/`: pnpm workspace, TypeScript strict, vitest
- CI
- Работающие механизмы инвариантов сборки S-1, S-2, S-3, S-5, S-6, S-10 — блокирующие с первого дня
- `AGENTS.md` в корне: реестр механизмов и Definition of Done
- Кодогенерация типов из схем и констант из реестра событий
- S-8 в режиме предупреждения

По разделу 9.11 патча v1.2 набор первого дня — это правила границ, реестр
событий с генерацией типов и файл правил исполнителя. Инвариант сборки
принимается **только вместе с работающей проверкой**, поэтому всё перечисленное
приезжает конфигами и тестами, а не текстом.

Sheets и порядок внедрения: [structural-invariants.md](../SPEC/structural-invariants.md).

## Чеклист приёмки

Отмечать только то, что проверено своими руками. Непроверенный пункт остаётся
пустым — именно из-за преждевременных галочек в 0.1.0 «DAG ацикличен» стоял
пройденным при живом цикле.

### Скелет и сборка

- [x] `pnpm install` проходит на чистой копии без ручных шагов
- [x] `pnpm typecheck` зелёный
- [x] `pnpm test` зелёный, и в выводе видно число тестов, а не «0 passed»
- [x] В `control-plane/packages/` шесть пакетов: contracts, domain, registry, snapshot, impact, cli
- [x] TypeScript strict включён вместе с `noUncheckedIndexedAccess` и `exactOptionalPropertyTypes`

### Кодогенерация

- [x] `pnpm gen` печатает число сгенерированных модулей схем и констант событий
- [x] Каталог `control-plane/packages/contracts/src/generated/` в `.gitignore` и не закоммичен
- [x] Ни один тип артефакта не объявлен руками: в `src/` вне `generated/` нет `interface Node`, `interface Edge` и подобного
- [x] `SCHEMA_NAMES` и тип `SchemaName` **генерируются**, а не написаны руками
- [x] `pnpm gen` чистит каталог вывода: устаревший модуль удалённой схемы не остаётся
- [x] Удаление схемы, на которую что-то опирается, роняет прогон **при чистом `generated/`** — то есть так же, как на CI. Проверить на `snapshot` (ломает кодогенерацию через `$ref`) и на `registry-bundle` (роняет тест манифеста и валидатор)
- [x] Ссылки bootstrap-манифеста разрешаются: удаление любой заявленной схемы, фикстуры или ключа роняет тест

### Механизмы инвариантов

Каждый пункт проверяется **негативно**: внести нарушение, увидеть падение, откатить.

- [x] S-1 — добавить `import { readFileSync } from "node:fs"` в `packages/domain` → тест границ падает
- [x] S-2 — добавить в `packages/registry` импорт из `@glt/impact` → тест границ падает
- [x] S-3 — вписать строку `"glt.snapshot.compiled"` в любой исходник → тест реестра событий падает
- [x] S-6 — создать цикл импортов между двумя модулями → тест границ падает
- [x] S-8 — добавить в `packages/domain` число вне `[-1, 0, 1, 2]` → тест параметров падает
- [x] S-10 — добавить команду в `COMMANDS` или строку в таблицу `cli.md` → тест поверхности CLI падает
- [x] В именах тестов стоят ID: `S-1`, `S-2`, `S-3`, `S-5`, `S-6`, `S-8`, `S-10`

### Поверхность CLI

- [x] Множество команд парсера в точности равно таблице в [cli.md](../SPEC/cli.md)
- [x] `commit`, `push`, `deploy`, `self-upgrade`, `self-write` отсутствуют как команды, подкоманды и флаги
- [x] Нереализованная команда завершается кодом 1 и **ничего** не пишет в stdout
- [x] Ни одна команда не пишет в рабочую копию (механизм — DEV-29, на DEV-01 только осмотр).

### Общее

- [x] CI зелёный
- [x] Изменение контракта записано в `glt-specpack/CHANGELOG.md`

### Чем проверить

```bash
pnpm install
pnpm gen
pnpm typecheck
pnpm test
pnpm boundaries
```

## Спецификация

- [architecture.md](../SPEC/architecture.md)
- [structural-invariants.md](../SPEC/structural-invariants.md)
- [cli.md](../SPEC/cli.md)
- [AGENTS.md](../../../AGENTS.md)

## Статус

**сделано** — ветка `wave1`, CI зелёный на чистом раннере
([run](https://github.com/Quantum-Insight-Lab/glt/actions/runs/34033887635)).

Чистая установка проверена 06.09: `node_modules` удалены полностью, затем
`pnpm install` → `pnpm typecheck` → `pnpm test` без единого ручного шага.
Постинсталл esbuild отрабатывает сам, тесты проходят.

До этой проверки шаг считался закрытым по локальному прогону на уже собранном
дереве, а это другое утверждение. `pnpm install` на существующем дереве печатает
«Already up to date» и не переоценивает build scripts, поэтому подтверждает
только то, что ничего не сломалось, — но не то, что установка с нуля работает.

Первый прогон CI упал, и упал поучительно: версия pnpm была объявлена дважды —
в `packageManager` и в конфиге workflow, — а `pnpm/action-setup` на такое
расхождение не соглашается. Два места, владеющие одним фактом, — ровно тот
дефект, против которого написан весь пакет, и он нашёлся в собственном CI.
Объявление из workflow удалено, единственным владельцем остался
`packageManager`.

Проверка чеклиста вскрыла ещё одно: тип `SchemaName` был написан руками и
дублировал список схем, а тест синхронности ловил удаление схемы только за счёт
устаревшего файла в каталоге вывода — то есть на CI, где каталога нет вообще, не
ловил никогда. `SCHEMA_NAMES` теперь генерируется, `pnpm gen` чистит вывод, а
ссылки bootstrap-манифеста проверяются отдельным тестом.

Собрано: pnpm workspace на шесть пакетов, TypeScript strict, vitest,
кодогенерация 12 модулей типов и 10 констант событий из пакета, CI и
блокирующие механизмы S-1, S-2, S-3, S-5, S-6, S-8, S-10.

Каждый механизм проверен негативно: нарушение внесено намеренно, прогон обязан
упасть. Шесть случаев, шесть падений. Механизм, видевший только чистый код, не
является доказательством.

Реализация вскрыла три расхождения со спекой, они записаны в `CHANGELOG.md`
0.7.0: `madge` удалён как второй детектор циклов (S-4), `cli` и `api` объявлены
композиционными корнями в S-2, S-3 и S-8 обеспечиваются сканом источников, а не
eslint.

Осмотр «не пишет в рабочую копию»: в `packages/cli/src` нет `writeFile`/`mkdir`;
`compile snapshot` и `impact` проверены тестами, что артефакт только в stdout.
