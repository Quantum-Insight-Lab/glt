...
- [x] То же для SourceRef внутри данных, не только во frontmatter
...
- [x] Есть тест с `S-7` в имени, если сверка реализуется здесь
...
`glt validate` и сверка S-7 переехали в репозиторий на DEV-04: схема-валидация пакета и фикстур — шаг CI, сверка реестров инвариантов блокирует. Тест с `S-7` в имени — `s7-coverage.test.ts`.

`lintSourceRef` — один проверщик (S-4) для frontmatter и для SourceRef внутри данных: бандл, `intended-components`, golden и прочие пути `defaultValidatePaths()`. Отсутствующий `path` — `source-ref-missing`. `INV-01` в имени теста — `lint-docs.test.ts`.
