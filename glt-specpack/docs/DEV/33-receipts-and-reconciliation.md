...
- [x] Попытка записывается **до** эффекта (PROTO-16)
- [x] Потеря связи после внешней записи даёт `unknown_outcome`, а не автоматический retry
- [x] Исход устанавливается сверкой с целевой системой и пишется новой audit-записью
- [x] `unknown_outcome` допустим как конечное состояние и не выдаётся за успех
...
**сделано** — ветка `wave4`. Попытка до эффекта; потеря связи — `unknown_outcome`, не retry; сверка — новая audit-запись; unknown не успех. CI: [run](https://github.com/Quantum-Insight-Lab/glt/actions/runs/35063282949).
