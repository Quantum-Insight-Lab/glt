...
describe("DEV-09 golden freeze and intended boundary", () => {
  ...
  it("PROTO-05 planned DEV step without code has expected_from_step", () => {
    ...
    const twentyThree = compiled.compiled.entries.find((e) => e.id === "glt.dev.23");
    expect(twentyThree).toBeDefined();
    const spec = twentyThree!.spec as { node?: { lifecycle?: string; delivery?: { expectedFromStep?: string } } };
    expect(spec.node?.lifecycle).toBe("planned");
    expect(spec.node?.delivery?.expectedFromStep).toBe("glt.dev.23");
  });
