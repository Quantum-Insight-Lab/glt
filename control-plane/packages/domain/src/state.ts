/**
 * Node state axes (DEV-16). Pure: snapshot fields, ages and optional signals
 * arrive already parsed. The evaluator classifies. It does not create
 * evidence, merge or write.
 *
 * Freshness vs P01 uses `snapshotIsStale` (S-4). Missing runtime is
 * `unknown`, never `healthy` (PROTO-12). Inference is never rewritten as
 * observation (INV-04).
 *
 * Contract: glt-specpack/docs/SPEC/topology.md
 */
...
export function evaluateState(input: EvaluateStateInput): TopologyState {
  ...
  const nodes = [...input.nodes]
    .map((node) => evaluateNode(node, freshness))
...
export function evaluateNode(node: NodeStateDraft, freshness: AxisValue<FreshnessState>): NodeState {
  ...
  return {
    ...
    axes: {
      ...
      runtime: bindAxis(node.runtime, "unknown"),
      freshness,
      ...
      conflict: bindAxis(node.conflict, "none"),
    },
  };
}
